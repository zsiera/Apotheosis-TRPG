...
public class MapGoal {
     ...
     * divided into type categories with parameters in order to simplify
     * parsing from XML.
     */
    public enum GoalType {    
        ELIMINATE_TARGETS("KILL TARGET"), 
        ROUTE_ENEMY("ROUTE ENEMY"), 
        DEFEND_TARGETS("DEFEND TARGET"), 
    ...
    public boolean isMet(Map map) { //TODO: Update to check turns
        if (type == GoalType.ELIMINATE_TARGETS) {
            ...
            boolean completed = true;
            for (int i = 0; i < targets.length; i++) {
                completed &= ((map.getUnit(targets[i]).getMapx() == tileX) && 
                        (map.getUnit(targets[i]).getMapy() == tileY));
            }
            return completed;
        }
    }
