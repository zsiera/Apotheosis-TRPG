...
public class Item {
    ...
    /*public Item(int typeindex) {
     switch (typeindex) {
     case 0: //Morphine Hypo
     icon = new Sprite(16, 16, 1, 1, SpriteSheet.ITEMICONSET);
     name = "MORPHINE H";
     used = new MenuAction() {
     @Override
     public void execute(MenuElement caller) {
     if (cursor.getSelectedUnit().getCurrentHp() >= 5) {
     cursor.getSelectedUnit().setCurrentHp(cursor.getSelectedUnit().getCurrentHp() - 5);
     } else {
     cursor.getSelectedUnit().setCurrentHp(0);
     }
     }
     };
     maxUses = 1;
        ...
        switch (type) {
            case ("HEAL"):
                ...
                used = new MenuAction() {
                    @Override
                    public void execute(MenuElement caller) {
                        cursor.getSelectedUnit().setCurrentHp(Math.max(Math.min(0, cursor.getSelectedUnit().getCurrentHp()
                                + magnitude), cursor.getSelectedUnit().
                                getHp()));
                    }
                };
                break;
