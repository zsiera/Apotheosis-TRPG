 ...
 * @author Evan
 *
 * Stores the array of tiles that make up a map, along with properties and
 * methods related to interfacing said map. Maps cannot be smaller than the
 * amount of tiles required to fill the screen (15 by 10 tiles).
 */
public class Map {
     ...
     * units.
     *
     * @param goalData The array of GoalData messenger objects for the map
     * @param unitData The array of UnitData messenger objects for the map
     */
    ...
    public Unit getSelectedUnit() {
        return getUnitTile(cursor.getMapX() + cursor.getMapY()
                * width);
    }
    ...
    public Sprite getMapSprite(int width, int height) {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                if (((int) Math.floor(x / tilePixelsX)) < this.width && ((int) Math.floor(y / tilePixelsY)) < this.height) {
                    mapSprite.setPixel(x + y * width, tiles[((int) Math.floor(x / tilePixelsX))
                            + ((int) Math.floor(y / tilePixelsY)) * this.width].getSprite().getPixel(((16 / tilePixelsX) * (x % tilePixelsX)) + ((16 / tilePixelsY) * (y % tilePixelsY))
                            * Tile.TILE_WIDTH));
                }
            }
        }
