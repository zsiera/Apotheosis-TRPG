 ...
 * @author Evan Stewart
 * 
 * A class containing a set of frames for an animation along with each frame's
 * corresponding sprite, duration, sound effects, etc.
 * 
 * TODO: Read from XML.
 */
public class Animation {
    ...
    public Animation(int iNumFrames, int[] frameDurationsIn) {
        ...
        if (frameDurationsIn.length < numFrames) {
            ...
            for (int i = frameDurationsIn.length; 
                    i < frameDurations.length; i++) {
                frameDurations[i] = 0;
            }
        } else {
            for (int i = 0; i < frameDurations.length; i++) {
     ...
     * @param frame: The frame corresponding to the desired sprite.
    ...
    /**
     * A method to be called on every update for an active animation.  
     * Increments the amount of updates that have occurred during the current 
     * frame, and increments the current frame when those exceed the frame
     * duration.
     **/
