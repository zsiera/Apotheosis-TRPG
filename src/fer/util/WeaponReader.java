...
public class WeaponReader {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < dirFiles.length; i++) {
            ...
            if (dirFiles[i].isDirectory() && "data".equals(dirFiles[i].getName())) {
        ...
        if (xmlFile != null) {
            try {
                ...
                for (int i = 0; i < weapons.size(); i++) {
                    ...
                    System.out.println("===Got " + properties.size() + " children===");
                    for (int j = 0; j < properties.size(); j++) {
                        if ("1".equals(properties.get(j).getValue()) && "index".equals(properties.get(j).getLocalName())) {
                            System.out.println("===Found index===");
                            System.out.println("===Index xml:===");
                            System.out.println(weapons.get(i).toXML());
                            XStream xstream = new XStream();
                            weapon = (WeaponData) xstream.fromXML(weapons.get(i).toXML());
                            System.out.println("===Serialized===");
                        }
                    }
                }
            } catch (ParsingException ex) {
                Logger.getLogger(WeaponReader.class.getName()).log(Level.SEVERE, null, ex);
