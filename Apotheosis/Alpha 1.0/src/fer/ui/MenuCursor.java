...
public class MenuCursor {
    ...
    public void update(int xScrollQueue, int yScrollQueue, boolean enter, boolean escape) {
        //Update the active element based upon the scroll queue
        do {
            if (elementIndex + yScrollQueue < 0) {
                elementIndex = activeMenu.getElements().size() + (elementIndex
                        + yScrollQueue);
            } else if (elementIndex + yScrollQueue >= activeMenu.getElements().size()) {
                elementIndex = (elementIndex + yScrollQueue) - activeMenu.
                        getElements().size();
            } else {
